...
suite("Response Certificate", func(){
  ...
  var content : Cbor.ContentMap = Cbor.load([]);
  var hashtree : Cbor.Tree = object {public func lookup(p: [Blob]): ?Blob = null};
  ...
  test("load cbor certificate", func(){
    let ?certificate = content.get<Cbor.ContentMap>("certificate", Cbor.getContentMap) else { trap("") };
    content := certificate
  });

  test("load hashtree from cert", func(){
    let ?tree = content.get<Cbor.Tree>("tree", Cbor.getHashTree) else { trap("") };
    hashtree := tree
  });
